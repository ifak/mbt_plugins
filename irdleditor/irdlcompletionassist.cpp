...
// ----------------------------
// IrdlCompletionAssistProvider
// ----------------------------
bool IrdlCompletionAssistProvider::supportsEditor(const Core::Id& editorId) const
{
  return editorId == Constants::IRDLEDITOR_ID;
}

IAssistProcessor* IrdlCompletionAssistProvider::createProcessor() const
{
  return new IrdlCompletionAssistProcessor;
}

int IrdlCompletionAssistProvider::activationCharSequenceLength() const
{
  return 1;
}

bool IrdlCompletionAssistProvider::isActivationCharSequence(const QString& sequence) const
{
  return (false ? sequence.isEmpty() : isActivationChar(sequence.at(0)));
}

// -----------------------------
// IrdlFunctionHintProposalModel
// -----------------------------
class IrdlFunctionHintProposalModel : public TextEditor::IFunctionHintProposalModel
{
public:
  IrdlFunctionHintProposalModel(QVector<model::base::SignalItem*> signalSymbols)
...
QString IrdlFunctionHintProposalModel::text(int index) const
{
...
int IrdlFunctionHintProposalModel::activeArgument(const QString &prefix) const
{
...
// -----------------------------
IrdlCompletionAssistProcessor::IrdlCompletionAssistProcessor()
  : _startPosition(0)
  , _keywordIcon(QLatin1String(":/mobata/images/base/type.png"))
  , _placeIcon(QLatin1String(":/mobata/images/irdl/place.png"))
  ...
  , _transitionIcon(QLatin1String(":/mobata/images/irdl/transition.png"))
...
QIcon IrdlCompletionAssistProcessor::proposalIcon(const int proposalType)
{
...
IAssistProposal* IrdlCompletionAssistProcessor::perform(const IAssistInterface* interface)
{
  this->_interface.reset(static_cast<const IrdlCompletionAssistInterface *>(interface));
  ...
  return this->createIrdlProposals(irdlText, afterRemoveLength,path);
}

IAssistProposal* IrdlCompletionAssistProcessor::createIrdlProposals(const QString& irdlText,
                                                                    ...
                                                                    const QString& path)
{
  ...
  ComCreateIrdlProposals proposalsCommand(irdlText, path, 0);
  QString errorString;
  bool result = proposalsCommand.execute(&errorString);
  if(!result)
    qDebug()<<"error during proposals creation: "<<errorString;

  if(proposalsCommand.irdlProposals().size() == 0)
    return nullptr;

  QChar lastCharBeforeCompletion;
  if(irdlText.length())
    lastCharBeforeCompletion=irdlText.at(irdlText.length()-1);

  for(const DslProposal& irdlProposal : proposalsCommand.irdlProposals())
  {
    QString proposalText = irdlProposal.proposalText;
    //    if(isIdentifierChar(lastCharBeforeCompletion))
    //      proposalText.insert(0, QLatin1Char(' '));

    this->addCompletion(proposalText,
                        this->proposalIcon(irdlProposal.proposalType),
                        afterRemoveLength);
  }
...
IAssistProposal* IrdlCompletionAssistProcessor::createContentProposal() const
{
...
IAssistProposal* IrdlCompletionAssistProcessor::createHintProposal(
    const QVector<model::base::SignalItem*>& symbols)
{
  IFunctionHintProposalModel *model = new IrdlFunctionHintProposalModel(symbols);
  IAssistProposal *proposal = new FunctionHintProposal(_startPosition, model);
  return proposal;
}

bool IrdlCompletionAssistProcessor::acceptsIdleEditor() const
{
...
void IrdlCompletionAssistProcessor::addCompletion(const QString &text,
                                                    const QIcon &icon,
                                                    int afterRemoveLength,
                                                    int order)
{
...
// -----------------------------
// IrdlCompletionAssistInterface
// -----------------------------
IrdlCompletionAssistInterface::IrdlCompletionAssistInterface(QTextDocument *textDocument,
                                                                 int position,
